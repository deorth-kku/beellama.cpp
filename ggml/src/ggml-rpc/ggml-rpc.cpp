...
// client) byte counts, plus a time-series of the two flows, and prints a
// summary at process exit. Enabled whenever GGML_RPC_DEBUG is set.
...
static std::vector<int64_t> g_rpc_traffic_ts_set; // cumulative set bytes per time bucket
static std::vector<int64_t> g_rpc_traffic_ts_get; // cumulative get bytes per time bucket
...
static bool is_pure_causal_mask(const void * data, ggml_type type, int64_t n_kv, int64_t n_rows, std::vector<int32_t> & boundaries) {
...
static void g_rpc_traffic_print(void) {
    ...
    // Time-series (trend across the run).
    GGML_LOG_DEBUG("--- time-series (64 buckets along run) ---\n");
...
static void ggml_backend_rpc_buffer_set_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    if (offset == 0
        ...
        if (is_pure_causal_mask(data, tensor->type, n_kv, n_rows, boundaries)) {
            ...
            memcpy(payload.data(), &rpc_tensor, sizeof(rpc_tensor));
            memcpy(payload.data() + sizeof(rpc_tensor), &offset, sizeof(offset));
            memcpy(payload.data() + sizeof(rpc_tensor) + sizeof(offset), &size, sizeof(size));
            uint32_t n_rows_u32 = (uint32_t) n_rows;
            memcpy(payload.data() + sizeof(rpc_tensor) + sizeof(offset) + sizeof(size), &n_rows_u32, sizeof(n_rows_u32));
            memcpy(payload.data() + sizeof(rpc_tensor) + sizeof(offset) + sizeof(size) + sizeof(uint32_t), boundaries.data(), (size_t) n_rows * sizeof(int32_t));
            bool status = send_rpc_cmd(ctx->sock, RPC_CMD_FILL_CAUSAL_MASK, payload.data(), payload.size());
            RPC_STATUS_ASSERT(status);
            // wire-traffic diagnostic: only the boundary payload went on the wire
            g_rpc_traffic_record(tensor->name, ctx->endpoint.c_str(), /*is_set=*/true, payload.size());
            return;
        }
...
static bool ggml_backend_rpc_kvarn_tail_attention_supported(
    ...
    rpc_msg_kvarn_tail_attention_supported_rsp response;
    bool ok = send_rpc_cmd(sock, RPC_CMD_KVARN_TAIL_ATTENTION_SUPPORTED,
...
static bool rpc_kvarn_bool_proc(ggml_backend_dev_t dev, enum rpc_cmd cmd, bool default_true) {
    ...
    rpc_msg_kvarn_proc_bool_rsp response;
    bool ok = send_rpc_cmd(sock, cmd, &request, sizeof(request), &response, sizeof(response));
    RPC_STATUS_ASSERT(ok);
...
static uint32_t ggml_backend_rpc_kvarn_native_rotated_max_query_tokens(ggml_backend_dev_t dev) {
    ...
    rpc_msg_kvarn_proc_u32_rsp response;
    bool ok = send_rpc_cmd(sock, RPC_CMD_KVARN_NATIVE_ROTATED_MAX_QUERY_TOKENS,
            &request, sizeof(request), &response, sizeof(response));
    RPC_STATUS_ASSERT(ok);
